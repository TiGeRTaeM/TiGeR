...
local function setRecoveryEmail(passwo
rd, new_recovery_email, dl_cb, cmd)
  tdcli_function ({
    ID = "SetRecoveryEmail",
    password_ = password,
    new_recovery_email_ = new_recovery_email
...
-- Returns information about a file, offline re
quest
...
-- UpdateChatTopMessage will not be sent, so returned message should be used to update chat t
op message
...
-- @bot_user_id Identifier of the bot send 
query to
...
-- Updates game score of the specified user in the game
-- @inline_message_id Inline message identifier
-- @edit_message True, if message should
 be edited
-- @user_id User identifier
-- @score New score
-- @force Pass True to update the score even if it decreases. If score is 0, user will be deleted from the high scores table
...
-- Updates updateFileProgress will
...
local function deleteSavedAni
mation(animation, dl_cb, cmd)
  tdcli_function ({
...
-- Returns user that can be contacted to get s
upport
...
-- Changes period of inactivity, after which the account of currently logged in user will be automatically dele
ted
-- @ttl New account TTL
...
-- @h
eight Height of the animation, may be replaced by the server
