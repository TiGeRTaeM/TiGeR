...
if matches[1] == "clear cache" or matches[1] == "Clear cache" then
     ...
     run_bash("rm -rf ~/.telegram-cli/data/p
     rofile_photo/*")
	...
	if matches[1]:lower() == 'savefile' and matches[2] and is_sudo(msg) or matches[1]:lower() == 'Savefile' and matches[2] and is_sudo(msg) then
		if msg.reply_id  then
            ...
            function get_filemsg(arg, data)
						...
						local cpath = tcpath..'/data/audio'
							if file_exi(audio_name, cpath) then
								...
								if lang then
									...
									tdcli.sendMessage(msg.to.id, msg.id_,1, '<b>Au
                  dio</b> <code>'..folder..'</code> <b>Has Been Saved.</b>', 1, 'html')
								end
							else
								if lang then
									tdcli.sendMessage(msg.to.id, msg.id_, 1, '_فایل مورد نظر وجود ندارد. فایل را دوباره ارسال کنید._', 1, 'md')
                ...
                tdcli_function ({
                 ID = 'GetMessage', chat_id_ = msg.chat_id_, message_id_ = data.id_ }, get_fileinfo, nil)
            end
...
-- #End By @MahDiRoO
  }, action_by_id, {chat_id=msg.to.id,user_id=matches[2]
