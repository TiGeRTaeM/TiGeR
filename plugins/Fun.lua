...
if cmd == 'moderator' and not is_mod(msg) or cmd == 'owner' and not is_owner(msg) then
 ...
 if matches[1]:lower() == "calc" and matches[2] or  matches[1]:lower()
 == "Calc" and matches[2] or  matches[1]:lower() == "حساب" and matches[2] then 
...
return tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, './Fun/ping.gif', '@TiGeeRTM', dl_cb, nil)
end
--------------------------------
...
-----------------
  if matches[1] == 'me' or matches[1] == 'Me' or matches[1] == 'من' then
    if is_sudo(msg) then
      return "*υšeяɳαɱҽ:* @"..(check_markdown(msg.from.username)
...
---------------
 if matches[1]:lower() == "photo" or  matches[1]:lower() == "Photo" or matches[1]:lower() == "عکس" then 
  ...
  tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "@TiGeeRTM", dl_cb, nil)
 end
