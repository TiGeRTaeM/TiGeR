...
redis محلی = {
    ...
    _DESCRIPTION =  ' یک کتابخانه مشتری Lua برای سیستم ذخیره سازی ارزش کلیدی redis. ' ،
    _COPYRIGHT    =  ' کپی رایت (C) 2009-2012 Daniele Alessandri ' ،
}

- خط زیر برای سازگاری با عقب برای استفاده از 'Redis' استفاده می شود
- نام ماژول جهانی با استفاده از "Redis" در حال حاضر نابهنگام است، بنابراین شما باید به صراحت اختصاص دهید
- ماژول به یک متغیر محلی هنگام نیاز به آن: 'local redis = require (' redis ') `.
ردیس = ردیس

محلی باز کردن =  _G . بسته بندی  یا میز. باز کردن
شبکه محلی ، درخواست، پاسخ = {}، {}، {}
...
تابع محلی merge_defaults ( پارامترها )
    اگر پارامترها ==  nil  پس از آن
        پارامترها = {}
    پایان
    برای k، v در  جفت (پیش فرض) انجام می شود
        اگر پارامتر [k] ==  nil باشد،  سپس
            پارامترهای [k] = پیش فرض [k]
        پایان
    پایان
    پارامترهای بازگشت
پایان

تابع محلی parse_boolean ( v )
    اگر v ==  ' 1 '  یا v ==  ' true '  یا v ==  ' TRUE '  سپس
        بازگشت  درست است
    elseif v ==  ' 0 '  یا v ==  ' false '  یا v ==  ' FALSE '  سپس
        بازگشت  نادرست
    چیز دیگری
        بازگشت  نزولی
    پایان
...
تابع محلی toboolean ( value ) return value ==  1  پایان

تابع محلی sort_request ( client ، command ، key ، params )
    - [[ params = {
        by = 'weight_ *'،
        دریافت = 'object_ *'،
        محدودیت = {0، 10}،
    ...
    اگر پارامز پس از آن
        اگر پارامز توسط  آن زمان
            table.insert (پرس و جو، ، با ، )
            table.insert (query، params. by )
        پایان

        اگر  نوع (params. limit ) ==  ' table ' باشد،  سپس
            - TODO: برای محدودیت های پایین و بالاتر بررسی کنید
            table.insert (پرس و جو، ، LIMIT : )
            table.insert (query، params limit [ 1 ])
            table.insert (query، params limit [ 2 ])
        پایان

        اگر پارامز سپس دریافت کنید 
            اگر ( نوع (params دریافت کنید ) ==  ' جدول ' ) پس از آن
...
تابع محلی zset_range_byscore_request ( client ، command ، ...)
    ...
    re
    quest.multibulk(client, command, args)
end
...
تابع network.read ( client ، len )
    ...
    اگر  نه اشتباه است و سپس  بازگشت خط دیگر مشتری. خطا ( ' خطای اتصال: '  
    .. err) پایان
پایان
    ...
    local function transaction(client, options, coroutine_block, attempts)
        ...
        if not raw_replies then
            if (retry or 0) <= 0 then
                ...
                return transaction(clien
                t, options, coroutine_block, retry - 1)
            end
        end
